# Tcell_filt_part1.R
# Score every cell for T / MNP / B / ILC lineage signatures from RNA, so that
# Tcell_filt_part2.R can classify and remove non-T cells.
#
# Usage:
#   Rscript Tcell_filt_part1.R <seurat_object.Rds> <genelist.csv> <ADT YES/NO>

args = commandArgs(TRUE)
sc_IGT_path   = args[1]
genelist_file = args[2]
ADT           = toupper(args[3]) == "YES"
...
pdf(file = "Tcell_cleanup_Step1_RNA_CellScoring.pdf", width = 19, height = 20)
...
saveRDS(subset_keep, file = "seuratobject_singlet_postRNAfiltering_postADTfiltering.Rds")
