# tcr_annotate_all_chains.R
# Attach per-cell clonotype + full TCR alpha/beta contig sequences to the
# singlet Seurat object, keeping ALL chains - productive AND nonproductive,
# paired AND unpaired (reads TCR/outs/all_contig_annotations.csv, the
# unfiltered CellRanger call). Submit the resulting FASTAs to IMGT
# High-V Quest for TCR annotation.
#
# Usage:
#   Rscript tcr_annotate_all_chains.R <EXP.txt> <seuratobject_singlet.Rds>
...
tcr_a <- tcr_a[,"full_a_seq", drop=FALSE]
tcr_b <- tcr_b[,"full_b_seq", drop=FALSE]
...
clono_seurat$EXP_cellID <- Cells(clono_seurat)
saveRDS(clono_seurat, "seuratobject_singlet_fullTCRinfo_productiveANDnonproductive_pairedANDunpaired.Rds")
...
alpha_contig1 <- data.frame(paste(paste0(clono_meta$EXP_cellID,'.ctg1'), paste0("V-Gene:", clono_meta$alpha_ctg1_v_gene), paste0("CDR3:", clono_meta$alpha_ctg1_cdr3), sep = " "), clono_meta$full_a_seq_contig1)
alpha_contig2 <- data.frame(paste(paste0(clono_meta_ctg2_alpha$EXP_cellID,'.ctg2'), paste0("V-Gene:", clono_meta_ctg2_alpha$alpha_ctg2_v_gene), paste0("CDR3:", clono_meta_ctg2_alpha$alpha_ctg2_cdr3), sep = " "), clono_meta_ctg2_alpha$full_a_seq_contig2)
...
# Cells with only a beta chain (no alpha) have contig == NA here - drop them
# so the fasta doesn't get an invalid ">header \n NA" entry.
alpha<-alpha[!is.na(alpha$contig), ]
alpha<-alpha[order(alpha$`cell_V-Gene_CDR3_alpha`), ]
write.csv(alpha, paste0(fasta_name, "alpha.csv"), row.names = T, quote = F)


beta_contig1 <- data.frame(paste(paste0(clono_meta$EXP_cellID,'.ctg1'), paste0("V-Gene:", clono_meta$beta_ctg1_v_gene), paste0("CDR3:", clono_meta$beta_ctg1_cdr3), sep = " "), clono_meta$full_b_seq_contig1)
beta_contig2 <- data.frame(paste(paste0(clono_meta_ctg2_beta$EXP_cellID,'.ctg2'), paste0("V-Gene:", clono_meta_ctg2_beta$beta_ctg2_v_gene), paste0("CDR3:", clono_meta_ctg2_beta$beta_ctg2_cdr3), sep = " "), clono_meta_ctg2_beta$full_b_seq_contig2)
...
# Cells with only an alpha chain (no beta) have contig == NA here - drop them
# so the fasta doesn't get an invalid ">header \n NA" entry.
beta<-beta[!is.na(beta$contig), ]
